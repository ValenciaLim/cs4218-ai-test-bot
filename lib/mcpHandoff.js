...
async function writePlaywrightHandoff(dir, payload) {
  ...
  const plannerMarkdownPaths = (payload.generatedSpecs || [])
    ...
    .map(e => e.specPath)
    .filter(Boolean);
  const playwrightSpecPaths = (payload.generatedSpecs || [])
    .filter(e =>
      /^tests\/.*\.spec\.(js|ts)$/i.test(String(e.specPath || "").replace(/\\/g, "/"))
    )
    .map(e => e.specPath)
    .filter(Boolean);
  const body = {
    ...
    conventions: {
      ...
      generatedThisRun: {
        ...
        playwrightSpecs: playwrightSpecPaths,
    ...
    playwrightTestAgents: {
      ...
      agents: [
        ...
        },
        {
          ...
          outputs: [
            "Executable Playwright tests under tests/ (e.g. tests/<StoryName>.spec.js). This bot may pre-write them from specs/*.md via LLM; refine with MCP against the live app."
          ],
          goal: "Align tests with the plan and live UI; patch locators and assertions via MCP where needed."
        },
        {
          id: "healer",
          label: "🎭 Healer",
    ...
    cursorAgentMust: [
      ...
      "Generator: Node may have written tests/*.spec.js per conventions.generatedThisRun.playwrightSpecs; refine or extend from specs/*.md using Playwright MCP against baseUrl.",
